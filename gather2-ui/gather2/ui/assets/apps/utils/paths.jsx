import { CORE_APP, ODK_APP, UI_APP } from './constants'

const API_PREFIX = ''
const APPS = [ CORE_APP, ODK_APP, UI_APP ]

/**
 * Returns the API url to get the Surveys data
 *
 * @param {string}  app          - app source: `core` (default), `odk-importer` or `ui`
...
export const getSurveysAPIPath = ({app, id, withStats, ...params}) => {
  const source = (APPS.indexOf(app) === -1 ? CORE_APP : app)
  const stats = (source === CORE_APP && withStats ? '-stats' : '')
...
export const getResponsesAPIPath = ({surveyId, ...params}) => {
  ...
  return `${API_PREFIX}/${CORE_APP}${surveyNested}/responses.${format}?${queryString}`
}
 ...
 *
 * @param {string}  app         - app source: `core`, `odk-importer` or `ui`
